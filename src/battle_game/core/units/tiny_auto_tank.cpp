...
AutoTank::AutoTank(GameCore *game_core, uint32_t id, uint32_t player_id)
...
void AutoTank::TankMove(float move_speed, float rotate_angular_speed) {
...
void AutoTank::TurretRotate() {
  auto player = game_core_->GetPlayer(player_id_);
  if (player) {
  	if( state_autoturret ){
  	  turret_rotation_ +=0.01f; 
	}
	else{
	  auto &input_data = player->GetInputData();
...
void AutoTank::Fire() {
  if (fire_count_down_ == 0) {
    ...
    if (player) {
      auto &input_data = player->GetInputData();
      if (input_data.mouse_button_down[GLFW_MOUSE_BUTTON_LEFT] || state_autofire ) {
...
bool AutoTank::IsHit(glm::vec2 position) const {
...
const char *AutoTank::UnitName() const {
  return "Tiny Auto Tank";
}

const char *AutoTank::Author() const {
